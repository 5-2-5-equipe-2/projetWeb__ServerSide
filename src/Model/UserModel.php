<?php
    ...
    class UserModel extends Database
    {
         ...
         * @throws DatabaseError
         */
        protected function getUserByIdUnSafe(int $id): array
        {
            return $this->select(
                "SELECT 
                                            {$this->getFields()}
        ...
        ): int
        {
            ...
            // create the user
            return $this->insert(
                "INSERT INTO user (username, password, first_name, surname, email, profile_picture) 
                                VALUES (?, ?, ?, ?, ?, ?)",
        ...
        public function updatePassword(int $userId, string $oldPassword, string $password): void
        {
            ...
            $this->validatePassword($password);
            $user = $this->getUserByIdUnSafe($userId);
         ...
         */
        public function searchUserChatroom(int $userId, string $search, int $limit = 10): array
        {
            $userManager = new UserManager();
            if ($userManager->getLoggedInUserId() != $userId) {
                throw new NotAuthorizedException();
            ...
            $data = $this->select("
                                        ...
                                        ON chat_room.id = chat_room_user.chat_room_id
                                        WHERE chat_room_user.user_id = ?
                                        AND (chat_room.name LIKE ?
                                        OR chat_room.description LIKE ?)
                                        ORDER BY chat_room.id
                                        LIMIT ?", ["issi", $userId, "%" . $search . "%", "%" . $search . "%", $limit]);
            if ($data == null) {
                return [];
            }
            return $data;
